    ...
    list=extend_schema(
        ...
        parameters=[
            ...
            OpenApiParameter(
                name='sort_by',
                type=str,
                description='Campo para ordenação (name, acronym)',
                required=False
            ),
            OpenApiParameter(
                name='sort_dir',
            ...
            OpenApiParameter(
                name='query',
                type=str,
                description='Termo de busca (nome do estado, sigla)',
                required=False
            )
        ]
    )
...
class StateViewSet(viewsets.GenericViewSet):
    ...
    def list(self, request):
        ...
        def build_url(target_page: int | None):
            ...
            return f"/api/states/?{'&'.join(params)}"

        return Response({
            'count': states['total'],
            'next': build_url(page + 1 if page < states['total_pages'] else None),
